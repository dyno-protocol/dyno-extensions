...
export default class MetamaskController extends EventEmitter {
  ...
  async addNewAccount() {
    ...
    if (!primaryKeyring) {
      throw new Error('DynoWalletController - No HD Key Tree found');
    }
    const { keyringController } = this;
  ...
  async verifySeedPhrase() {
    ...
    if (accounts.length < 1) {
      throw new Error('DynoWalletController - No accounts found');
    }
  ...
  async signMessage(msgParams) {
    log.info('DynoWalletController - signMessage');
    ...
    try {
      ...
      log.info('DynoWalletController - eth_sign failed', error);
  ...
  async signPersonalMessage(msgParams) {
    log.info('DynoWalletController - signPersonalMessage');
    const msgId = msgParams.metamaskId;
    // sets the status op the message to 'approved'
    // and removes the metamaskId for signing
    try {
      ...
      log.info('DynoWalletController - eth_personalSign failed', error);
  ...
  async decryptMessageInline(msgParams) {
    log.info('MDynoWalletController - decryptMessageInline');
  ...
  async decryptMessage(msgParams) {
    log.info('DynoWalletController - decryptMessage');
    const msgId = msgParams.metamaskId;
    // sets the status op the message to 'approved'
    ...
    try {
      ...
      log.info('DynoWalletController - eth_decrypt failed.', error);
  ...
  async encryptionPublicKey(msgParams) {
    log.info('DynoWalletController - encryptionPublicKey');
    const msgId = msgParams.metamaskId;
    // sets the status op the message to 'approved'
    // and removes the metamaskId for decryption
    try {
      ...
      log.info(
        'DynoWalletController - eth_getEncryptionPublicKey failed.',
        error,
      );
  ...
  async signTypedMessage(msgParams) {
    log.info('DynoWalletController - eth_signTypedData');
    const msgId = msgParams.metamaskId;
    ...
    try {
      ...
      log.info('DynoWalletController - eth_signTypedData failed.', error);
