...
const initApp = async (remotePort) => {
  ...
  log.info('DynoWallet initialization complete.');
};
...
async function loadStateFromPersistence() {
  ...
  if (versionedData && !versionedData.data) {
    ...
    sentry.captureMessage('DynoWallet - Empty vault found - unable to recover');
  }
  ...
  if (!versionedData) {
    throw new Error('DynoWallet - migrator returned undefined');
  }
  ...
  if (localStore.isSupported) {
    ...
    setTimeout(() => {
      throw new Error('DynoWallet - Localstore not supported');
    });
  }
...
function setupController(initState, initLangCode, remoteSourcePort) {
  ...
  pump(
    ...
    (error) => {
      log.error('DynoWallet - Persistence pipeline failed', error);
    },
  );
  ...
  async function persistData(state) {
    if (!state) {
      throw new Error('DynoWallet - updated state is missing');
    }
    if (!state.data) {
      throw new Error('DynoWallet - updated state does not have data');
    }
    if (localStore.isSupported) {
      try {
