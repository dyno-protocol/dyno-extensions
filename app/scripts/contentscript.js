...
function injectScript(content) {
  try {
    ...
    console.error('DynoWallet: Provider injection failed.', error);
  }
}
...
async function setupPhishingStream() {
  ...
  pump(pageMux, pageStream, pageMux, (err) =>
    logStreamDisconnectWarning('DynoWallet Inpage Multiplex', err),
  );
  pump(extensionMux, extensionStream, extensionMux, (err) => {
    logStreamDisconnectWarning('DynoWallet Background Multiplex', err);
    window.postMessage(
      {
...
async function setupStreams() {
  ...
  pump(legacyPageMux, legacyPageStream, legacyPageMux, (err) =>
    logStreamDisconnectWarning('DynoWallet Legacy Inpage Multiplex', err),
  );
  pump(
    legacyExtensionMux,
    extensionStream,
    ...
    legacyExtensionMux,
    (err) => {
      logStreamDisconnectWarning('DynoWallet Background Legacy Multiplex', err);
      notifyInpageOfStreamFailure();
    },
  );
...
function forwardTrafficBetweenMuxes(channelName, muxA, muxB) {
  ...
  pump(channelA, channelB, channelA, (error) =>
    console.debug(
      `DynoWallet: Muxed traffic for channel "${channelName}" failed.`,
      error,
    ),
  );
}
...
function forwardNamedTrafficBetweenMuxes(
  ...
  pump(channelA, channelB, channelA, (error) =>
    console.debug(
      `DynoWallet: Muxed traffic between channels "${channelAName}" and "${channelBName}" failed.`,
      error,
    ),
  );
}
...
function logStreamDisconnectWarning(remoteLabel, error) {
  console.debug(
    `DynoWallet: Content script lost connection to "${remoteLabel}".`,
    error,
  );
}
...
function redirectToPhishingWarning(data = {}) {
  console.debug('DynoWallet: Routing to Phishing Warning page.');
